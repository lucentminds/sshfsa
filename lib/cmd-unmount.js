...
module.exports = function( o_args ){
   ...
   if( !c_host && !o_args.all ) {
      ...
      console.error( 'Try \'sshfsa --help\' for more information.' );
      process.exit( 1 );
   }
   ...
   .then(function( a_mounts ){
      ...
      if( !!o_args.all ){
         ...
         .then(( a_results )=>{
            const a_errors = a_results.filter(( c_result )=>{
               return !!c_result;
            });

            if( a_errors.length > 0 ){
               return Promise.reject( a_errors );
            }
         });
      }
...
const unmount_sshfs_mount = function( o_sshfs_mount ){
   return new Promise(function( resolve, reject ){
      ...
      child.stderr.on( 'data',function( c_data ){
         c_result = c_data.toString( 'utf8' ).trim();
         resolve( c_result );
      });
